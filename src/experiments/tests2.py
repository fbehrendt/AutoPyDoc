...
def resolve_variable_chain_by_file(code_representer, filename="src\experiments\\ast_tests.py", variable_chain="var_B.var_class_a.func_d"):
...
def resolve_variable_chain(code_representer, content, variable_chain, full_path):
    ...
    for line in content:
        ...
        if line.lstrip().strip('self.').startswith(variable_chain.split('.')[0]):
            if line.lstrip().strip('self.').lstrip(variable_chain.split('.')[0]).lstrip().startswith('='):
                ...
                if '.' in identified_source.split('(')[0]: # case solve right side of assignment
                    ...
                    if re.fullmatch(pattern=pattern, string=string):
                        return resolve_variable_chain(code_representer=code_representer, content=content, variable_chain='.'.join(variable_chain.split('.')[1:], full_path=full_path))
                    ...
                    if remaining_variable_chain != "":
                        if code_obj.type != "class":
                            raise Exception("Trying to access variable in method from outside")
                        result_id = resolve_variable_chain(code_representer=code_representer, content=code_obj.code.split('\n'), variable_chain='.'.join(variable_chain.split('.')[1:]), full_path=full_path)
                        return result_id
                    else:
                        return code_obj.id
            ...
            if line.lstrip().startswith("def " + variable_chain.split('.')[0]): # TODO also .lstrip('(') ?
                # get code_obj
                ...
                if method_id in code_representer.objects.keys():
                    ...
                    remaining_variable_chain = '.'.join(variable_chain.split('.')[1:])
                    if remaining_variable_chain != "":
                        result_id = resolve_variable_chain(code_representer=code_representer, content=code_obj.code.split('\n'), variable_chain=remaining_variable_chain, full_path=full_path)
                        return result_id
                    else:
                        return code_obj.id
                else:
                    raise Exception(f"code_obj with id {method_id} not found in code_representer")
    raise Exception(f"Could not resolve chain. {remaining_variable_chain} not found in {content}")    
