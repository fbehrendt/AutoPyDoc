    ...
    def check_outdated(
        ...
        try:
            with self.gpt_model.chat_session():
                prompt = self.build_check_outdated_prompt(
                    existing_docstring, code_generation_input
                )
                ...
                docstring_matches = self.extract_check_outdated_output(generated_text)

                return not docstring_matches
        except Exception as e:
            self.logger.exception(
                "An error occurred during checking existing docstring", exc_info=e
            )
            raise e

    def build_check_outdated_prompt(
...
"""
    ...
    def extract_check_outdated_output(self, result: str) -> bool:
        match = re.search(
    ...
    def generate_docstring(self, code_generation_input: str) -> dict:
        try:
            with self.gpt_model.chat_session():
                prompt = self.build_generate_docstring_prompt(code_generation_input)
                ...
                docstring_matches = self.extract_generate_docstring_output(
                    generated_text
                )
    ...
    def build_generate_docstring_prompt(self, code_content, language="english") -> str:
        return f"""
...
"""
    ...
    def extract_generate_docstring_output(self, result: str) -> dict:
        match = re.search(
