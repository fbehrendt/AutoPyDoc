...
def validate_docstring_input(
    docstring_input: DocstringInput, code_representer=CodeObject
):
    ...
    if isinstance(docstring_input, DocstringInputMethod):
        # check description
        if (
            isinstance(docstring_input.description, bool)
            and not docstring_input.description
        ):
            pr_notes.append(
        ...
        for param in docstring_input.arguments.keys():
            ...
            if (
                ...
                pr_notes.append(
                    f"Please manully add a description for parameter {param} in {get_rel_filename(code_obj.filename)}->{generate_parent_chain(code_obj=code_obj, code_representer=code_representer)}"
                )
        ...
        if code_obj.return_type is not None:
            ...
            if (
                isinstance(docstring_input.return_type, bool)
                and not docstring_input.return_type
            ):
                pr_notes.append(
        ...
        for instance_attribute_name in docstring_input.instance_attributes.keys():
            ...
            if (
                ...
                and not docstring_input.instance_attribute_types[
